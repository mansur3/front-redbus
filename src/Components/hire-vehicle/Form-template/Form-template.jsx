...
// end of form data
    ...
    const time_dest = async () => {
      ...
      let response = await fetch(`https://api.distancematrix.ai/maps/api/distancematrix/json?origins=${pick_up}&destinations=${dest}&departure_time=now&key=${process.env.REACT_APP_DISTANCE_TOKEN}`)
  ...
  const getData = async () => {
    let res_bus = await fetch(`${process.env.REACT_APP_URL}/bus/all`);
    let res_tempo = await fetch(`${process.env.REACT_APP_URL}/tempo/all`);
    let res_car  = await fetch(`${process.env.REACT_APP_URL}/car/all`);
